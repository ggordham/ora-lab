...
refresh=FALSE
ora_scrpt_list="oraDBCA.sh oraRWLRun.sh oraRWLSetup.sh oraTNS.sh oraLsnr.sh oraDBSamp.sh oraTools.sh oraPDBClone.sh oraDBSrvc.sh ora-lab-login.sh"
...
if [ "${refresh}" == "FALSE" ]; then
    ...
    if [ "$TEST" == "TRUE" ]; then
        ...
        /usr/bin/nohup /bin/bash -c "/usr/bin/sleep ${1} && /usr/bin/sudo -u root /usr/sbin/reboot" _ "${wait_sec}" >> /tmp/ora-lab-reboot.log 2>&1 &
