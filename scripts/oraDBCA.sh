...
if checkopt_oraDBCA "$OPTIONS" ; then
    ...
    # variables=
    ORACLE_BASE_HOME=/u01/app/oracle/homes/DB_21_8,
    DB_UNIQUE_NAME=t3db,
    ...
    response_file="${SCRIPTDIR}/dbca_${ora_db_sid}.rsp"
...
datafileDestination=${ora_data_dir}/{DB_UNIQUE_NAME}/
...
# Misc other options
variablesFile=
variables=ORACLE_BASE_HOME=/u01/app/oracle/homes/DB_21_8,DB_UNIQUE_NAME=t3db,ORACLE_BASE=/u01/app/oracle,PDB_NAME=,DB_NAME=t3db,ORACLE_HOME=/u01/app/oracle/product/21/dbhome_1,SID=t3db
...
EOF
    ...
    # run DBCA tool
    dbca_options=" -silent -createDatabase"
 
    if [ "$TEST" == "TRUE" ]; then logMesg 0 "dbca command: $ORACLE_HOME/bin/dbca $dbca_options -responseFile $response_file" I "NONE" ; fi
    "${ORACLE_HOME}/bin/dbca" $dbca_options -responseFile $response_file
