...
case "${1}" in
  "setuek")
      ...
      uek_to_use=$( /usr/sbin/grubby --info=ALL | /usr/bin/grep ^kernel | /usr/bin/grep uek | cut -d= -f2 | head -1 )
  ...
  "setrhck")
      ...
      rhck_to_use=$( /usr/sbin/grubby --info=ALL | /usr/bin/grep ^kernel | /usr/bin/grep -Ev "uek|rescue" | cut -d= -f2 | /usr/bin/head -1 )
